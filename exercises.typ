...
By Theorem 1.11 $alpha = inf A$ exist in $RR$. This means that $alpha lt.eq x$ for any $x in A$, or equivalently $-x lt.eq -alpha$. Since $-x in -A$ it follows that $-alpha$ is an upper bound to $-A$.
...
This shows that $gamma$ cannot be an upper bound to $-A$ since $-y in -A$. Therefore $-alpha = sup(-A)$ and

$
    inf A = alpha = -(-alpha) = -sup(-A),
$
...
which means that $z in N_r (x)$ and therefore $x$ is a limit point of $E$ so that $x in E'$.
...
Let ${G_alpha}$ be any open cover of $K$. Then we know that there is an index $alpha_0$ such that $0 in G_(alpha_0)$. Since $G_(alpha_0)$ is an open set, we know that there exists a neighborhood $N$ around $0$ with radius $r > 0$ such that $N subset G_(alpha_0)$.

If $r gt.eq 1$, then $G_(alpha_0)$ covers $K$ and there is nothing to prove. Assume therefore that $r < 1$. By Archimedean property of $RR$ we can find a positive integer $p$ such that $r gt.eq 1 slash p$. Let $m$ be the smallest non-negative integer such that $r gt 1 slash m$. It follows that if $q$ is an integer where $q > m$, then $1 slash q = abs(1 slash q - 0) < r$ so that $1 slash q in N$. This shows that there are at most $m-1$ points in $E$ that are not in $N_r$
...
Let $E_n$ be the set of all numbers generated by the sequence ${x_k}$ along with the point $1 / n$. Then clearly $E_n$ has only one limit point, namely $1 / n$​. We give an example, if $n=1$ we have
...
We have constructed a compact set $S$ of real numbers whose limit points $S'$ form a countable set as desired.
...
Now that we have shown that our $y$ is irrational we need only to prove that for any $x in E$ the number $z = x + y$ is also irrational.
...
Suppose, to get a contradiction, that the sum $z = x + y$ is also rational.  It follows then that $z$ is a repeating decimal (any "terminating" number can be viewed as a repeating decimal where $0$s being repeated). Let $sigma$ be the length of the repetition and $z_i$ the first decimal where the repetition starts.
...
$ underbrace(x_k + y_k, z_k) = underbrace(x_(k + b sigma) + y_(k + b sigma), z_(k + b sigma)). $
...
Since $x in E$ we know that each decimal of $x$ is either $4$ or $7$. However, relation @2.17-rel cannot be satisfied using any combination of these two digits and therefore $z$ cannot be a repeating decimal. This contradicts our assumption about $z$ and the sum $z = x + y$ is thus irrational for every $x in E$.
...
Since $X$ is separable, it contains a countable dense subset $E$. For every $p in E$ let $W_p$ consists of all neighborhoods that has a positive rational radius $h > 0$

$
    W_p = {N_h (p) | h in rationals, h > 0 }.
$

Each member of $W_p$ is associated with exactly one positive rational radius. This means that $W_p$ is countable. By the Corollary to Theorem 2.12 the union
...
$E$ is dense in $X$ and therefore we can find a point $p in E$ such that the distance $d(p, x) = delta < r / 2$. Since $rationals$ is dense in $RR$, there exists an $h in rationals$ such that $delta < h < r / 2$ by Theorem 1.20.
...
which implies $y in N_r (x)$ and therefore $V_alpha subset N_r (x) subset G$. Since the choice of $x$ and $G$ were arbitrary it follows that the collection ${V_alpha}$ is a countable base of $X$.
