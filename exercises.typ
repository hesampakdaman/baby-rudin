...
Hence, the sequence ${x_k}$ is generated by successively taking midpoints between the previous term $x_(k-1)$ and $1/n$. Let $E_n$ be the set of all numbers generated by the sequence ${x_k}$. For example, for $n=2$ we have

$ E_2 = { 5/12, 11/24, 23/48, dots }. $
...
We can show that any other point $y in reals$ such that $y eq.not 1 / n$ cannot be a limit point of $E_n$. Since $E_n$ is a countable set, there is a smallest number $x_k eq.not y$ in $E_n$ such that $d(y, x_k)$ is minimized. Then the neighborhood with $0 < r < d(y, x_k)$ will have no point of $E_n$ (other than possibly $y$). Hence, $1/n$ is the only limit point of $E_n$ and its closure is

$ overline(E)_n = E_n union { 1 / n }. $

Since $overline(E)_n subset (1/(n+1), 1/n]$ we see that $overline(E)_n$ is bounded.

By construction of $E_n$, since we take midpoints $E_n sect E_m = emptyset$ if $m eq.not n$.

Let $E$ be the set

$ E = union.big_(n=1)^infinity overline(E)_n. $
