...
Hence, the sequence ${x_k}$ is generated by successively taking midpoints between the previous term $x_(k-1)$ and $1/n$.
...
Since the sequence ${x_k}$ is constructed by repeatedly taking midpoints between the previous term and $1/n$​, the sequence will approach the value $1/n$​, making it a limit point of $E_n$. Because $E_n subset (1/(n+1) 1/n]$ it follows that $E_n$ is bounded by $1/n$.

We can show that any other point $y in reals$ such that $y eq.not 1 / n$ cannot be a limit point of $E_n$. Since $E_n$ is a countable set of values that approach $1/n$, there is a smallest number $x_k eq.not y$ in $E_n$ such that $d(y, x_k)$ is minimized. Then the neighborhood with $0 < r < d(y, x_k)$ will have no point of $E_n$ (other than possibly $y$). Hence, $1/n$ is the only limit point so that $ E'_n = {1 / n}$.
...
$ S = {0} union union.big_(n=1)^infinity E_n. $
